...
class GraphPanel extends JPanel {
    ...
    public GraphPanel(ArrayList<Double> dataPoints) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < numberYDivisions + 1; i++) {
            ...
            if (dataPoints.size() > 0) {
                ...
                g2.setColor(Color.BLACK);
                String yLabel = ((int) ((getMinDataPoint()
                        + (getMaxDataPoint() - getMinDataPoint()) * ((i * 1.0) / numberYDivisions)) * 100)) / 100.0
                        + "";
        ...
        for (int i = 0; i < dataPoints.size(); i++) {
            if (dataPoints.size() > 1) {
                int x0 = i * (getWidth() - padding * 2 - labelPadding) / (dataPoints.size() - 1) + padding
                        + labelPadding;
                int x1 = x0;
    ...
    private double getMinDataPoint() {
        double minDataPoint = Double.MAX_VALUE;
        for (Double dataPoint : dataPoints) {
            minDataPoint = Math.min(minDataPoint, dataPoint);
        }
        return minDataPoint;
    }

    private double getMaxDataPoint() {
        double maxDataPoint = Double.MIN_VALUE;
        for (Double dataPoint : dataPoints) {
